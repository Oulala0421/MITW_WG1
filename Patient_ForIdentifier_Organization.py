...
def check(m,n,line):
    ...
    if type(m[n]) == dict:
        ...
        if n == "resourceType" and m[n] == "Patient":
            ALL.append({"id":"","identifier":"","active":"","name":"","gender":"","birthDate":"","managingOrganization":""})
        if n == "id":
            ALL[k]["id"] = m[n]
        if n == "active":
            ALL[k]["active"] = m[n]
...
window=tk.Tk()
window.title("Organization identifier Search")
window.geometry("1000x600+250+15")



label = tk.Label(window, text = 'Organization')
label.pack()
var = tk.StringVar()
...
word=tree["columns"]=("identifier","active","name","gender","birthDate","managingOrganization")
...
def bt():
    ...
    ForIdentifier = var.get()
    #url = "https://oauth.dicom.org.tw/fhir/Patient?organization="+ForIdentifier
    url = "http://192.168.50.3:10610/gateway/fhir/Patient?organization="+ForIdentifier
...
def main():
    ...
    for i in range(len(ALL)):
        tree.insert("",i,text=ALL[i]["id"],values=(ALL[i]["identifier"],ALL[i]["active"],ALL[i]["name"],ALL[i]["gender"],ALL[i]["birthDate"],ALL[i]["managingOrganization"]))
